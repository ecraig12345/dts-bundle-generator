...
export function generateOutput(params: OutputParams, options: OutputOptions = {}): string {
	...
	if (params.imports.size) {
		...
		const importsArray = sortedEntries.flatMap(([libraryName, libraryImports]) =>
			generateImports(libraryName, libraryImports)
		);
	...
	if (options.sortStatements) {
		statements.sort((a, b) => a.sortingValue.localeCompare(b.sortingValue));
	}
	...
	if (params.wrappedNamespaces.size) {
...
function getStatementText(
	...
	const needResolveIdentifiers = !ts.isExportDeclaration(statement) || !statement.moduleSpecifier;
	...
	const printer = ts.createPrinter(
		...
		},
		{
			...
			substituteNode: (hint: ts.EmitHint, node: ts.Node) => {
				...
				if (ts.isImportTypeNode(node) && node.qualifier && helpers.needStripImportFromImportTypeNode(node)) {
				...
				const resolvedStatementName =
					(nodeName && helpers.resolveIdentifierName(nodeName as ts.Identifier)) || undefined;
				...
				modifiersMap[ts.SyntaxKind.ExportKeyword] = shouldHaveExportKeyword;
...
function generateImports(libraryName: string, imports: ModuleImportsSet): string[] {
	...
	Array.from(imports.requireImports)
		.sort()
		.forEach(importName => result.push(`import ${importName} = require('${libraryName}');`));
	Array.from(imports.defaultImports)
		.sort()
		.forEach(importName => result.push(`import ${importName} ${fromEnding}`));

	if (imports.namedImports.size) {
		result.push(
...
function generateReferenceTypesDirective(libraries: string[]): string {
	return libraries
		.sort()
		.map(library => `/// <reference types="${library}" />`)
		.join('\n');
}
